...
class FaqCatalog extends Component {
  state = {
    ...
    initOffset: mediaQuery(200, 200, 270),
  }

  sections = {}

  getSections(catalog) {
    return catalog.catalog.map(({ name, slugs }) => (
      <div id={name} key={name} ref={node => (this.sections[name] = node)}>
        <A_H type="section" mx={cn('section')}>
          {name}
        </A_H>
  ...
  getQuestion = slug => {
    ...
    return (
      <div className={cn('item')} key={slug}>
        <div
          className={cn('item-header', { 'is-opened': isOpened })}
          onClick={() => this.setState({ opened })}
        >
          <A_H type="faq" key={slug}>
            {slug}
          </A_H>
        </div>
        <article
          dangerouslySetInnerHTML={{ __html: catalog.entities[slug] }}
          className={cn('item-article', { 'is-opened': isOpened })}
        />
      </div>
    )
  }
  ...
  getNavigation(catalog, selected) {
    return catalog.catalog.map(({ name }, i) => {
      return (
        <A_Link
          key={name}
  ...
  componentDidMount() {
    window.addEventListener('resize', this.recalculateMediaQuery)
    document.addEventListener('scroll', this._handleUserScroll)
  }

  componentWillUnmount() {
    document.removeEventListener('scroll', this._handleUserScroll)
    window.removeEventListener('resize', this.recalculateMediaQuery)
  }

  resizeId = null

  //This function is needed to update offset after window resizing
  recalculateMediaQuery = () => {
    clearTimeout(this.resizeId)
    this.resizeId = setTimeout(() => {
      this.setState({ initOffset: mediaQuery(200, 200, 270) })
    }, 200)
  }
  ...
  _handleUserScroll = () => {
    ...
    const selectedIndex = keys.findIndex((key, i, arr) => {
      if (i === 0) {
        return (
          this.sections[key].getBoundingClientRect().top > top ||
          this.sections[keys[i + 1]].getBoundingClientRect().top > top
        )
      } else {
        return (
          this.sections[key].getBoundingClientRect().top <= top &&
          (!arr[i + 1] || this.sections[keys[i + 1]].getBoundingClientRect().top > top)
        )
      }
    })

    const selected = this.props.catalog.catalog[selectedIndex].name
    const stickyDisabled = this.sections[keys[keys.length - 1]].getBoundingClientRect().bottom < 300
  ...
  startScrollMotion = (name, i) => {
    const offset = i === 0 ? 0 : this.sections[name].offsetTop + this.state.initOffset
    window.onscroll = window.onwheel = e => {
      e.preventDefault()
    }
    this.setState({
    ...
    return (
      <div ref={node => (this.scrollContainer = node)}>
        ...
        <A_Container type="normal" mx={cn('container')}>
          <O_Fixed mx={cn('aside')} top={mediaQuery(0, 70, 0)} disabled={stickyDisabled}>
            <div className={cn('aside-menu')}>
              <A_H type="small-section">Contents</A_H>
              {navigation}
            </div>
          </O_Fixed>
          <div className={cn('main', { hide })}>{sections}</div>
          {scrollMotionActive && (
            <Motion
              defaultStyle={{
                scrollMotion:
                  (this.sections[Object.keys(this.sections)[0]].getBoundingClientRect().top - 350) *
                  -1,
                // scrollMotion: currentScrollPosition //from
              }}
              style={{
                scrollMotion: spring(scrollMotionTo, {
                  stiffness: 100,
                  damping: 20,
                  precision: 10,
                }), //to
